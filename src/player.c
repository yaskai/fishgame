...
void PlayerInput(Entity *player, float dt) 
{
	...
	if(player->flags & ENT_ORBIT) 
	{
		...
		{
			if(p->jump_timer > 0 && !p->input->jump)
				PlayerEndJump(player, true);
		}
...
void PlayerPhysicsOrbit(Entity *player, float dt) 
{
	...
	//player->orbit_data.angular_vel = (p->orbit_vel.x / (dt * 1000)) + player->orbit_angle;
	//player->orbit_data.angular_vel = (p->orbit_vel.x) + player->orbit_angle;

	//float t_impact = (player->orbit_height / p->orbit_vel.y);
	player->orbit_data.angular_vel = (p->orbit_vel.x) + player->orbit_angle;
